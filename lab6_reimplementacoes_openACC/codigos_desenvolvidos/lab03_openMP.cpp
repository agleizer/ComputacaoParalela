...
void multiplicarMatrizesPorBlocagem(const std::vector<std::vector<int>>& A, 
...
int main(int argc, char* argv[]) {
    ...
    if (argc > 1) {
        numThreads = std::atoi(argv[1]); // Converte argumento para inteiro
        if (numThreads < 1) {
            std::cerr << "Número de threads inválido! Usando 1 thread." << std::endl;
            numThreads = 1;
        }
    }
    ...
    std::cout << "Tempo de execucao (blocagem, OpenMP) com " << numThreads << " threads: " 
